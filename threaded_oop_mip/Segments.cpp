...
pair<double, double> Segments::get_exon_segment_loss(vector<Alignment>::iterator alignment, set<unsigned long> overlap_region, bool is_best) {
    ...
    for (id_int = curr_intron_ids.begin(); id_int != curr_intron_ids.end(); id_int++) {
        ...
        unsigned long intron_start = this->intron_ids[*id_int]->start;
        unsigned long intron_end = intron_start + this->intron_ids[*id_int]->length - 1;
        ...
        // compute loss with and without current alignment
        loss_with += compute_mip_loss(intron_cov_with, this->intron_ids[*id_int]->expectation);
        loss_without += compute_mip_loss(intron_cov_without, this->intron_ids[*id_int]->expectation);
    }
