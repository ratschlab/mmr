...
python $sample -s -F -q ${workdir}/error_model/quality_uniform.sample $noise -G $gio_file -m ${workdir}/error_model/mismatch.pickle -o ${filebase}${noisetag}.optimal -f ${filebase}.fasta -b ${filebase}.bed | gzip -c -6 > "${filebase}${noisetag}".fastq.gz
